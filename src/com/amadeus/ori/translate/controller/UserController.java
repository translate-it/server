...
public class UserController implements Controller {
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		if (action.equals("listUsers")) {
			...
			final String isGoogleAccount = request
					.getParameter("isGoogleAccount");
			...
			if (isGoogleAccount != null) {
				...
				return createUser(username, password,
						Boolean.parseBoolean(isGoogleAccount));
			...
			if (StringUtils.isEmpty(username)) {
				...
				return updateUser(username, password, firstName, lastName,
						email, languages, projects, bundles, isEnabled, isAdmin);
			}
			...
			if (StringUtils.isEmpty(subject) || StringUtils.isEmpty(message)
					|| StringUtils.isEmpty(receipients)) {
	...
	private ModelAndView createUser(String username, String password,
			boolean isGoogleAccount) throws Exception {
	...
	private ModelAndView updateUser(String username, String password,
			String firstName, String lastName, String email, String languages,
			String projects, String bundles, String isEnabled, String isAdmin)
	...
	// Send email
	private ModelAndView sendMail(String subject, String message,
			String[] receipients) {

		AdminResultMessage res = new AdminResultMessage();
		try {
			// LOG.info("Got a contact request from: " +
			// req.getParameter("email") + ", " + req.getParameter("name"));

			Session session = Session
					.getDefaultInstance(new Properties(), null);
			Message msg = new MimeMessage(session);
			msg.setFrom(new InternetAddress("lab.amadeus@gmail.com",
					"TranslateIT Administrator"));
			...
			msg.addRecipient(Message.RecipientType.TO, new InternetAddress(
					"innovation@amadeus.com", "Innovation"));

			// put the contents as HTML/Mime Body
			MimeBodyPart htmlPart = new MimeBodyPart();
			htmlPart.setContent(
					"<span style=\"font-family:sans-serif;color:#1F497D\">"
							+ message
			...
			logEntryRepository
					.addEntry("Message was sent to users: " + subject);
